...
export default () => {
    ...
    const urlApiLocal = '/v1/token/cancel-sync';

    it("Cancel Calendar Synchronization: Bad request", ( done ) => {
    ...
    it("Cancel Calendar Synchronization: success", ( done ) => {
        let headers = {
        ...
        .then((json: any) => {    
            ...
            mock(json);
            expect(mock).toBeCalledWith(expect.objectContaining({ 
                affectedTokens: expect.any(Number)
            }));       
            done();
        })
        .catch((err: any) => {
